# In file: UAV_Env/gym_foo/foo_env.py

import gymnasium as gym  # Use gymnasium, the new standard
...
class SecrecyISACEnv(gym.Env):
    ...
    def __init__(self, w_S=0.5, Train=True, multiUT=True, LoadData=False, 
        ...
        # =========================================================================
        # 2. DEFINE STATE AND ACTION SPACES (CORRECTED)
        # =========================================================================
        # State: [isac_xy(2), jammer_xy(2), all_cu_xy(4), time(1)]
        ...
        # Action: [isac_v(2), jammer_v(2), p_isac(1), p_jammer(1), ris_phases(M)]
        ...
        # This is a good practice, though Gymnasium's reset behavior is more robust
        self.reset()

    # In Gymnasium, reset() now returns (obs, info)
    def reset(self, seed=None, options=None):
        super().reset(seed=seed) # Important for new Gymnasium versions
        ...
        self.ris_phases = self.np_random.uniform(-np.pi, np.pi, self.NUM_RIS_ELEMENTS)
        self.current_step = 0
        
        initial_obs = self._get_state()
        initial_info = {} # Start with an empty info dictionary
    ...
    def _calculate_rician_channel_vec(self, pos_ris, pos_rx):
        ...
        # Use self.np_random for reproducibility, as is standard in Gymnasium
        nlos_rand = (self.np_random.standard_normal(size=self.NUM_RIS_ELEMENTS) + 1j * self.np_random.standard_normal(size=self.NUM_RIS_ELEMENTS)) / np.sqrt(2)
        h_nlos = np.sqrt(h_nlos_gain) * nlos_rand
    ...
    def step(self, action):
        ...
        # In Gymnasium, the 'done' flag is split into 'terminated' and 'truncated'
        terminated = False # You would set this to True if a terminal state is reached (e.g., agent crashes)
        truncated = self.current_step >= self.MAX_STEPS # This is True if the episode ends due to a time limit
