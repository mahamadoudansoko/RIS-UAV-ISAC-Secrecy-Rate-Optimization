...
TOTAL_TIMESTEPS = 10000  # Set to a low value for debugging, increase for real results
output_dir = "results"
...
for w_S in W_S_VALUES:
    ...
    test_env = gym.make('foo-v0', w_S=w_S)
    test_env = DummyVecEnv([lambda: test_env])

    isac_pos_log, jammer_pos_log, secrecy_rate_log, sensing_snr_db_log = [], [], [], []

    obs, info = test_env.reset() # Gymnasium reset returns (obs, info)
    done = False
    while not done:
        ...
        # The 'done' flag is now the logical OR of terminated and truncated
